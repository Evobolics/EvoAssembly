...
namespace Root.Code.Libs.Mono.Cecil
{
	...
	public sealed class FieldDefinition : FieldReference, IMemberDefinition, IConstantProvider, IMarshalInfoProvider {
		...
		int offset = Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker;

		internal int rva = Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker;
		byte [] initial_value;

		object constant = Root.Code.Libs.Mono.Cecil.Mixin.NotResolved;

		MarshalInfo marshal_info;

		void ResolveLayout ()
		{
			if (offset != Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker)
				return;

			if (!HasImage) {
				offset = Root.Code.Libs.Mono.Cecil.Mixin.NoDataMarker;
				return;
			}
		...
		void ResolveRVA ()
		{
			if (rva != Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker)
				return;
		...
		public bool HasConstant {
			get {
				...
				return constant != Root.Code.Libs.Mono.Cecil.Mixin.NoValue;
			}
			set { if (!value) constant = Root.Code.Libs.Mono.Cecil.Mixin.NoValue; }
		}
