...
using System.Text;
using Root.Code.Libs.Mono;
using Root.Code.Libs.Mono.Cecil.Cil;
using Root.Code.Libs.Mono.Cecil.Metadata;
using Root.Code.Libs.Mono.Cecil.PE;
using Root.Code.Libs.Mono.Collections.Generic;
using FileAttributes = Root.Code.Libs.Mono.Cecil.FileAttributes;
	...
	sealed class ImmediateModuleReader : ModuleReader {
		...
		public void ReadModule (ModuleDefinition module, bool resolve_attributes)
		{
			...
			if (module.HasAssemblyReferences)
				Root.Code.Libs.Mono.Cecil.Mixin.Read (module.AssemblyReferences);
			if (module.HasResources)
				Root.Code.Libs.Mono.Cecil.Mixin.Read (module.Resources);
			if (module.HasModuleReferences)
				Root.Code.Libs.Mono.Cecil.Mixin.Read (module.ModuleReferences);
			if (module.HasTypes)
				ReadTypes (module.Types);
			if (module.HasExportedTypes)
				Root.Code.Libs.Mono.Cecil.Mixin.Read (module.ExportedTypes);
		...
		void ReadType (TypeDefinition type)
		{
			...
			if (type.HasLayoutInfo)
				Root.Code.Libs.Mono.Cecil.Mixin.Read (type.ClassSize);
		...
		void ReadGenericParameters (IGenericParameterProvider provider)
		{
			...
			for (int i = 0; i < parameters.Count; i++) {
				...
				if (parameter.HasConstraints)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (parameter.Constraints);
		...
		void ReadSecurityDeclarations (ISecurityDeclarationProvider provider)
		{
			...
			for (int i = 0; i < security_declarations.Count; i++) {
				...
				Root.Code.Libs.Mono.Cecil.Mixin.Read (security_declaration.SecurityAttributes);
			}
		}
		...
		void ReadCustomAttributes (ICustomAttributeProvider provider)
		{
			...
			for (int i = 0; i < custom_attributes.Count; i++) {
				...
				Root.Code.Libs.Mono.Cecil.Mixin.Read (custom_attribute.ConstructorArguments);
			}
		}
		...
		void ReadFields (TypeDefinition type)
		{
			...
			for (int i = 0; i < fields.Count; i++) {
				...
				if (field.HasConstant)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (field.Constant);

				if (field.HasLayoutInfo)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (field.Offset);

				if (field.RVA > 0)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (field.InitialValue);

				if (field.HasMarshalInfo)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (field.MarshalInfo);
		...
		void ReadMethods (TypeDefinition type)
		{
			...
			for (int i = 0; i < methods.Count; i++) {
				...
				if (method.HasOverrides)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (method.Overrides);

				if (method.IsPInvokeImpl)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (method.PInvokeInfo);
				...
				if (return_type.HasConstant)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (return_type.Constant);

				if (return_type.HasMarshalInfo)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (return_type.MarshalInfo);
		...
		void ReadParameters (MethodDefinition method)
		{
			...
			for (int i = 0; i < parameters.Count; i++) {
				...
				if (parameter.HasConstant)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (parameter.Constant);

				if (parameter.HasMarshalInfo)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (parameter.MarshalInfo);
		...
		void ReadProperties (TypeDefinition type)
		{
			...
			for (int i = 0; i < properties.Count; i++) {
				...
				Root.Code.Libs.Mono.Cecil.Mixin.Read (property.GetMethod);

				if (property.HasConstant)
					Root.Code.Libs.Mono.Cecil.Mixin.Read (property.Constant);
		...
		void ReadEvents (TypeDefinition type)
		{
			...
			for (int i = 0; i < events.Count; i++) {
				...
				Root.Code.Libs.Mono.Cecil.Mixin.Read (@event.AddMethod);
	...
	sealed class MetadataReader : ByteBuffer {
		...
		void CompleteTypes ()
		{
			...
			for (int i = 0; i < types.Length; i++) {
				...
				Root.Code.Libs.Mono.Cecil.Mixin.Read (type.Fields);
				Root.Code.Libs.Mono.Cecil.Mixin.Read (type.Methods);
			}
		}
		...
		public Row<short, int> ReadTypeLayout (TypeDefinition type)
		{
			...
			if (!metadata.ClassLayouts.TryGetValue (rid, out class_layout))
				return new Row<short, int> (Root.Code.Libs.Mono.Cecil.Mixin.NoDataMarker, Root.Code.Libs.Mono.Cecil.Mixin.NoDataMarker);
		...
		public int ReadFieldLayout (FieldDefinition field)
		{
			...
			if (!metadata.FieldLayouts.TryGetValue (rid, out offset))
				return Root.Code.Libs.Mono.Cecil.Mixin.NoDataMarker;
