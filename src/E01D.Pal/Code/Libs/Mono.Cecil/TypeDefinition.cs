...
namespace Root.Code.Libs.Mono.Cecil
{
	...
	public sealed class TypeDefinition : TypeReference, IMemberDefinition, ISecurityDeclarationProvider {
		...
		short packing_size = Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker;
		int class_size = Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker;
		...
		void ResolveLayout ()
		{
			if (packing_size != Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker || class_size != Root.Code.Libs.Mono.Cecil.Mixin.NotResolvedMarker)
				return;

			if (!HasImage) {
				packing_size = Root.Code.Libs.Mono.Cecil.Mixin.NoDataMarker;
				class_size = Root.Code.Libs.Mono.Cecil.Mixin.NoDataMarker;
				return;
			}
	...
	public sealed class InterfaceImplementation : ICustomAttributeProvider
	{
		...
		{
			Root.Code.Libs.Mono.Cecil.Mixin.CheckType (interfaceType, Root.Code.Libs.Mono.Cecil.Mixin.Argument.interfaceType);
